...
object ThumbnailIngest extends Controller with Thumbnail {
  ...
  def ingest(path: String): Result = {
    ...
    if (!dir.exists() || !dir.isDirectory) {
      ...
      val thumbnails: Seq[(Option[ObjectId], File)] = (images.map {
        image => (storeThumbnail(image, 180), image)
      }).toList ::: (images.map {
        image => (storeThumbnail(image, 500), image)
      }).toList
      val failures = thumbnails.filter(_._1 == None).map {
        r => r._2.getAbsolutePath
      }
      if (failures.size == 0) {
